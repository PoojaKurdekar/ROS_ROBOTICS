...
def OdomCallback(message,cargs):
    ...
    pub,msg,goal = cargs
    # Tunable parameters
    wgain = 70.0 # Gain for the angular velocity [rad/s / rad]
    vconst = 130.0 # Linear velocity when far away [m/s]
    ...
    # Reporting
    print('OdomCallback: x=%4.1f,y=%4.1f distx=%4.2f, disty=%4.2f, cmd.vx=%4.2f,cmd.vy=%4.2f, cmd.w=%4.2f\n'%(pose[0],pose[1],distancex,distancey,vx,vy,w))
########################################
# Main Script
# Initialize our node
...
rospy.Subscriber('/odom',nav_msgs.msg.Odometry,OdomCallback,
                 (cmdpub,cmdmsg,goal))
